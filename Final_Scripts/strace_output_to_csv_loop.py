...
selected_threads=[44,48,52]


for i in selected_threads:
  output_file='strace_output_combined_2T_4C_'+ str(i) +'_threads.txt'
  loop_no=1
  ...
  with open(output_file,'r') as file:
    for line in file: # read each line
      if skip<2:
        skip+=1 # skipping lines which contain '---'/ lines which aren't system call and their count in the strace summary output
        continue
      arr=line.split()
      values[column_names.index(arr[-1])]+=int(arr[3]) # use the system call to find index position in the array and add the system calls of all clients for that thread per run
      if "write" in line: 
        write_count+=1
        if write_count == 4: # at every 4th encounter of the write call (since summary output is sorted), push output of each loop run as a row in the dataframe
          loop_no+=1
...
df.to_csv('System_calls_2T_4C.csv') # save it to a csv file
